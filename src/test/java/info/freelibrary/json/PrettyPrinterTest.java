...
public class PrettyPrinterTest extends AbstractTestBase {
    ...
    @Test
    public void testObjectsAndArrays() throws IOException {
        final PrettyWriter output = new PrettyWriter(new StringWriter());
        final JsonObject contents = new JsonObject();

        contents.add("id", "dc6c77").add("type", "ATService");

        new JsonObject().add("service", new JsonArray().add(contents)).writeTo(output);
        assertEquals(
                "{\n  \"service\" : [\n    {\n      \"id\" : \"dc6c77\",\n      \"type\" : \"ATService\"\n    }\n  ]\n}",
                output.toString());
    }
